...
export default function Home() {
  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Header */}
      <header className="bg-card border-b border-border shadow-warm px-6 sm:px-10 lg:px-16 py-4 flex items-center justify-between">
            ...
            <div className="absolute top-0 left-[8%] w-[360px] bg-card rounded-2xl border border-border shadow-warm-lg p-5 z-20 rotate-2 hover:scale-110 hover:rotate-3 hover:z-30 transition-all duration-200 ease-in-out cursor-default">
              ...
              <div className="mt-3.5">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs text-muted-foreground">Items claimed</span>
                  <span className="text-xs font-medium text-foreground">12 of 20</span>
                </div>
              ...
              {/* Divider */}
              <div className="border-t border-dashed border-border my-3.5" />
              {/* Footer */}
              <div className="flex items-center justify-between">
                <div className="flex -space-x-1.5">
                  {[
                    ...
                    { letter: "R", bg: "bg-terracotta-100 text-terracotta-500" },
                  ].map(({ letter, bg }) => (
                ...
                <span className="text-xs text-muted-foreground">
                  <span className="font-medium text-foreground">9 going</span> · 3 maybe
                </span>
              </div>
            </div>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={day} className="flex items-center gap-2.5">
                    <div className={`text-center rounded-lg px-1.5 py-1 min-w-[40px] ${winner ? "bg-primary text-white" : "bg-muted text-foreground"}`}>
                      <p className="text-[9px] font-bold uppercase tracking-widest leading-none opacity-80">{month}</p>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                        <div className={`h-full rounded-full ${winner ? "bg-primary" : "bg-sand-400"}`} style={{ width: pct }} />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={label} className="flex items-center gap-2">
                    ...
                    {person && <span className="text-[11px] text-muted-foreground">{person}</span>}
                  </div>
                ))}
              </div>
            </div>
