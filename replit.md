...
- May 12, 2026. Gated commenting on RSVP: guests must RSVP before leaving item notes or discussion messages; RSVP name becomes their identity across the site; the name input field is replaced by a "Commenting as {name}" label; hosts are exempt; polling-mode events (no RSVPs collected yet) are also exempt. Backend enforces the same rule server-side.
- May 12, 2026. Added per-item comment threads (collapsible "notes" under each item row with count badge) and an event-level discussion/Q&A panel. New itemComments + eventComments tables in DB. All comments real-time via WebSocket. Host can delete any comment; others can delete their own by name match. Author name pre-filled from auth user or localStorage.
